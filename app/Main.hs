...
main = do
	   ...
	   print $ decode students assignments $ hungarianMethodBoxed graph
	 --  print graph
...
decode :: [String] -> [String] -> ([(Int, Int)], Int) -> [String]
decode p a (res, cost) = decode' p a res
  where
    decode' p a res = map (\(person, assignment) -> p !! (person - 1) ++ ": " ++ a !! (assignment - 1)) res
